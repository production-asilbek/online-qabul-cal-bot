...
export function AppointmentCard({
  ...
  const { t } = useI18n();
  const staffLabel = display.staff.title.toLowerCase().includes("dentist")
    ? `${t.dentistPrefix} ${display.staff.firstName}`
  ...
  const content = (
    <Card className="flex items-start gap-4">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 text-sm text-[var(--tg-subtitle-text-color)]">{staffLabel}</div>
        <div className="mt-2">
